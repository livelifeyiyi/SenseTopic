...
selected_user = selected_user[0:2000]


class PMF(object):
	def __init__(self, topic_file, topic_type, time_num, rootDir, num_feat=100, epsilon=1, _lambda=1, momentum=0.8, maxepoch=1000, num_batches=20, batch_size=1000):
		...
		print("Reading neighbors_flag.json file......")
		with codecs.open(rootDir + 'neighbors_flag_0.json', mode='r') as infile:
			self.neighbors_0 = json.load(infile)
		with codecs.open(rootDir + 'neighbors_flag_1.json', mode='r') as infile:
			self.neighbors_1 = json.load(infile)
		...
		if (not incremental) or (self.w_Item is None):
			...
			if self.topic_type == 'DMM':
				...
				for doc_id, topic_num in enumerate(codecs.open(self.topic_file, mode='r', encoding='utf-8')):
					if topic_num:
						topic = [0 for i in range(self.num_feat)]
...
if __name__ == "__main__":
	...
	print("Reading Rijt_rating_nozero.npy file......")
	...
	pmf = PMF(topic_file=rootDir+"topic_assign_user2000", topic_type="DMM", time_num=31, rootDir=rootDir)
